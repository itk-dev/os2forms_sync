...
final class WebformHelper {
   ...
   *
   * @return \Drupal\webform\Entity\WebformInterface[]
   *   The webforms.
   *
   * @phpstan-return array<int, WebformInterface>
   */
  ...
  public function webformToArray(WebformInterface $webform): array {
    ...
    $url = Url::fromRoute('os2forms_sync.webform.show', ['webform' => $webform->id()],
  ...
  public function webformThirdPartySettingsFormAlter(array &$form, FormStateInterface $form_state): void {
    ...
    $form['third_party_settings']['os2forms']['os2forms_sync']['publish'] = [
      ...
      '#description' => $this->t('If checked this form will be listed on <a href=":url_index">:url_index</a> and details made available on <a href=":url_show">:url_show</a>', [
        ':url_index' => Url::fromRoute('os2forms_sync.webform.index')->toString(),
        ':url_show' => Url::fromRoute('os2forms_sync.webform.show', ['webform' => $webform->id()])->toString(),
      ]),
    ];

    if ($info = $this->importHelper->getWebformImportInformation($webform)) {
      $form['third_party_settings']['os2forms']['os2forms_sync']['sync_info'] = [
        '#markup' => $this->t('Webform imported from <a href=":url">:url</a> at @imported_at.', [
          ':url' => $info['source_url'],
